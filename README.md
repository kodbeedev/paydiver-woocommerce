...
[Jomabee](https://kodbee.com) payment gateway by **Kodbee**.

- Redirect-based hosted checkout (no card data on your site)
- Automatic order completion via signed (HMAC-SHA256) webhook
- Works with any Jomabee instance

## Install

1. Copy this folder to `wp-content/plugins/jomabee-woocommerce` (or upload the zip).
2. Activate **Jomabee for WooCommerce** in WordPress → Plugins.
3. **WooCommerce → Settings → Payments → Jomabee** → enter:
...
2. After payment, Jomabee calls the store webhook (`/?wc-api=jomabee`).
3. The plugin verifies the `X-Jomabee-Signature` against the raw body and calls
